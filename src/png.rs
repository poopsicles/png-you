...
impl Png {
    ...
    fn remove_chunk(&mut self, chunk_type: &str) -> Result<Chunk, Error> {
        match self
            ...
            .iter()
            .enumerate()
            .find(|&(_, chunk)| chunk.chunk_type() == &chunk_type.parse::<ChunkType>().unwrap())
        {
            None => Err(Error::ChunkNotFound),
            Some((i, _)) => Ok(self.chunks.remove(i)),
        }
    }
    ...
    fn chunk_by_type(&self, chunk_type: &str) -> Option<&Chunk> {
        self.chunks
            .iter()
            .find(|&chunk| chunk.chunk_type() == &chunk_type.parse::<ChunkType>().unwrap())
    }

    fn as_bytes(&self) -> Vec<u8> {
        let mut res = Vec::from(Self::MAGIC_NUMBER);

        for chunk in self.chunks.iter() {
            res.extend_from_slice(&chunk.as_bytes());
        }
...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(value: &[u8]) -> Result<Self, Self::Error> {
        ...
        loop {
            let len_arr = value.take(4);
            match len_arr.len() { 
                0 => break, // end of buffer
                4 => (), // cool
                _ => return Err(Error::NotEnoughData)
            }

            let length = u32::from_be_bytes(&len_arr.try_into().unwrap());
        }
        ...
        //         Err(IoError{repr: ErrorKind::UnexpectedEof}) => return Err(Error::NotEnoughData),
        //         Err(ErrorKind) => return Err(Error::Unknown),
        //         Ok(_) => ()
        //     }
        //     let length = u32::from_be_bytes(length);

        //     let mut chunk = Vec::with_capacity(12 + length as usize);
        //     if let Err(ErrorKind::UnexpectedEof) = reader.read_exact(&mut chunk) {
        //         return Err(Error::NotEnoughData)
        //     }
...
impl Display for Png {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        for chunk in self.chunks.iter() {
            res.push_str(&chunk.data_as_string().unwrap());
            res.push('\n');
        }
...
mod tests {
    ...
    fn test_valid_from_bytes() {
        ...
        let bytes: Vec<u8> = Png::MAGIC_NUMBER
            .iter()
